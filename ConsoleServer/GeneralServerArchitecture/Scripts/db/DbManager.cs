...
public class DbManager 
{
    ...
    {
        //创建MySqlConnection 对象
        mysql = new MySqlConnection();

        //连接参数
        string s = string.Format("server={0};port={1};user={2};password={3};database={4};",ip, port, user, pw, db );
        //string s = "server=127.0.0.1;port=3306;user=root;password=;database=game;";
    ...
    public static bool CheckPassword(string id, string pw) 
    {
        ...
        if (!DbManager.IsSafeString(id))
        {
            Console.WriteLine("[数据库CheckPassword fail, id not safe");
            return false;
        }
        if (!DbManager.IsSafeString(pw))
        {
